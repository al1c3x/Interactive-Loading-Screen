...
void TextureManager::loadBatchStreamAsset(int index, IExecutionEvent* execEvent, ThreadPool* pool)
{
	int fileNum = 0;
	threadPool = pool;
	...
	for (const auto& entry : std::filesystem::directory_iterator(STREAMING_PATH)) {
		...
		{
			String path = entry.path().generic_string();
			threadPool->scheduleTask(new StreamAssetLoader(path, execEvent));
			break;
		}
		fileNum++;
	}
